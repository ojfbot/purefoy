...
def print_coverage_report(report: CoverageReport, delta: Optional[Dict[str, Any]] = None):
    ...
    # Summary stats
    print(f"  {Colors.BOLD}Forums:{Colors.RESET}        {Colors.BRIGHT_GREEN}{report.forums_covered}{Colors.RESET}/{report.total_forums}")
    print(f"  {Colors.BOLD}Topics:{Colors.RESET}        {Colors.BRIGHT_GREEN}{report.topics_scraped:,}{Colors.RESET}/{report.total_topics:,}")
    ...
    if delta and delta.get('new_posts', 0) > 0:
        ...
        print(f"     {Colors.GREEN}+{delta['new_topics']:,}{Colors.RESET} topics, {Colors.GREEN}+{delta['new_posts']:,}{Colors.RESET} posts")
...
def print_summary_table(report: CoverageReport):
    ...
    print(f"  {Colors.BOLD}OVERALL{Colors.RESET:13} {draw_progress_bar(report.overall_coverage_percent, width=20)} {overall_pct}")
    print()
...
def print_recommendations(report: CoverageReport):
    ...
    incomplete_forums = [
        f for f in report.forums.values()
...
def print_quick_stats(report: CoverageReport):
    ...
    print(f"{Colors.BOLD}Coverage:{Colors.RESET} {pct} | "
